...
class FakeRunner:
    ...
    def preprocess_with_original(self, _):
        raise ValueError("test path")


@unittest.skipUnless(_HAS_TESTCLIENT, "fastapi TestClient requires httpx")
class PredictContractTests(unittest.TestCase):
    def test_predict_returns_500_on_runner_failure(self):
        app = create_app(
            runner=FakeRunner(),
        ...
        resp = client.post("/predict", files={"file": ("x.jpg", b"abc", "image/jpeg")})
